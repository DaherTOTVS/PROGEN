...
User Function ACD166DH()
 ...
 Upd := " UPDATE "+RetSqlName("SDC")+" SET D_E_L_E_T_ = '*',DC_OP=DC_PEDIDO WHERE R_E_C_N_O_ IN (SELECT R_E_C_N_O_ FROM "+RetSqlName("SDC")+" WHERE D_E_L_E_T_ <>'*' AND DC_SEQ NOT IN ( "
 Upd +=" SELECT C9_SEQUEN FROM "+RetSqlName("SC9")+" WHERE D_E_L_E_T_ <>'*' AND C9_PEDIDO=DC_PEDIDO "
 ...
 Upd := "UPDATE "+RetSqlName("SDC")+" SET D_E_L_E_T_='*',DC_OP=DC_PEDIDO WHERE D_E_L_E_T_ <> '*' "
...
Upds := "UPDATE "+RetSqlName("SBF")+" SET BF_EMPENHO=0,BF_ESTFIS='1' WHERE D_E_L_E_T_ <> '*' "
...
Upds := "UPDATE "+RetSqlName("SBF")+"  SET BF_EMPENHO=1,BF_ESTFIS='2' WHERE D_E_L_E_T_ <> '*' "
Upds += " AND  EXISTS(SELECT 1 FROM "+RetSqlName("SDC")+"  WHERE D_E_L_E_T_ <>'*' AND DC_PRODUTO=BF_PRODUTO AND BF_LOCAL=DC_LOCAL AND BF_NUMSERI=DC_NUMSERI) "
