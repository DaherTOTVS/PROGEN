...
/*/
user Function XA200Subst(aAutoCab, aAutoItens)
...
Static Function A200PrSubs(cCodOrig, cGrpOrig, cOpcOrig, cCodDest, cGrpDest, cOpcDest, aAutoItens, aAutoCab)
	...
	Else
		...
		Else
			...
			elseif slRevAut 
				// cFilSG1 += " AND (SELECT COUNT(*) FROM " + RetSqlName('SB1') + " SB1 WHERE B1_COD=G1_COD AND SB1.D_E_L_E_T_ = ' ' AND (B1_REVATU=G1_REVFIM OR G1_REVFIM='ZZZ')) > 0"
				cFilSG1 += " AND 1=1"
			else		
				cFilSG1 += " AND 1=1"
			EndIf	
				...
				For nI := 1 to Len(aOrdens)
					...
					If aOrdens[nI][1] == .T.
						...
						If slUsaSBZ // VERIFICA SE O PARAMETRO ESTÁ SETADO COM A SBZ
							cLocal:= GetAdvFVal("SBZ","BZ_LOCPAD",xFilial("SBZ")+aDadosDest[1],1,"")  
						EndIf
						// ALTERAÇÃO MATEUS HENGLE - 23/06/23
