...
Static Function ReportDef()
	...
	Local cTitle	:= 'Ordenes de Produccion' //"Ordens de Producao"
	Local nlargDesc	:= 0
	...
	TRCell():New(oSection1,'C2_NUM'		,'SC2','ORDEN DE PRODUCCION NRO :'   ,PesqPict('SC2','C2_NUM')	 ,TamSX3('C2_NUM')[1]	  ,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	TRCell():New(oSection1,'C2_EMISSAO' ,'SC2','Emision'   ,PesqPict("SC2","C2_EMISSAO")	,10						,/*lPixel*/,/*{|| DTOC(dDataBase) }*/)
	...
	TRCell():New(oSection1,'OpQuant'  	,'SC2','Cantidad'   ,PesqPict("SC2","C2_QUANT")	,TamSX3("C2_QUANT")[1]	,/*lPixel*/,{|| (cAliasTop)->C2_QUANT - (cAliasTop)->C2_QUJE })
	...
	TRCell():New(oSection1,'C2_ROTEIRO'	,'SC2','Guión de operaciones',/*Picture*/					,/*Tamanho*/			,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	oSection1:Cell('C2_ROTEIRO'):SetCellBreak()
	...
	//³ Sessao 2 (oSection2)                                         ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oSection2 := TRSection():New(oSection1,STR0051,{"SD4","SB1","SG1","NNR"},/*Ordem*/) //"Empenhos"
	oSection2:SetHeaderBreak()
	oSection2:SetReadOnly()

	TRCell():New(oSection2,'D4_COD'	 	,'SD4','Codigo'   ,PesqPict('SD4','D4_COD')    ,25/*nSize*/,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,.T./*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,.F./*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	TRCell():New(oSection2,'B1_DESC' 	,'SB1','Descripción'   ,PesqPict('SB1','B1_DESC')   ,30 ,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,.T./*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,/*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	TRCell():New(oSection2,'G1_OSERV' , ,'Observación'  , ,20 ,/*lPixel*/, {||_xDesg1((cAliasTop)->C2_PRODUTO,SD4->D4_COD)},/*cAlign*/,.T./*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,/*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	TRCell():New(oSection2,'D4_QTDEORI'  	,'SD4','Cant original'   ,PesqPict('SD4','D4_QTDEORI')    ,TamSX3('D4_QTDEORI')[1]+4    ,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,/*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,/*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	TRCell():New(oSection2,'D4_QUANT' 	,'SD4','Cantidad'   ,PesqPict('SD4','D4_QUANT')  ,TamSX3('D4_QUANT')[1]  ,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,/*lLineBreak*/,/*cHeaderAlign*/,/*lCellBreak*/,/*nColSpace*/,/*lAutoSize*/,/*nClrBack*/,/*nClrFore*/)
	...
	TRCell():New(oSection4,'QtdeProd'	,'SH8',/*STR0048*/ 'Cantidad Producida'   ,/*Picture*/, 12       ,/*lPixel*/,{|| Space(12) })
	TRCell():New(oSection4,'QtdPerda'	,'SH8',/*STR0049*/'Pérdidas'   ,/*Picture*/,/*Tamanho*/,/*lPixel*/,{|| Space(12) })
	TRCell():New(oSection4,'VldInici'   ,'SH8',/*STR0074*/'Validez inicial','@!', 10,/*lPixel*/,/*{|| code-block de impressao }*/) //"Validade inicial"
	TRCell():New(oSection4,'VldFinal'   ,'SH8',/*STR0075*/'Validez Final','@!', 10,/*lPixel*/,/*{|| code-block de impressao }*/,/*cAlign*/,.T.) //"Validade Final"
	...
	oSection2:SetNoFilter({"SD4","SB1","SB2","SC2","SG1"})
...
Static Function ReportPrint(oReport, cAliasTop)
	...
	ElseIf !oReport:oPage:IsLandScape()
		...
		oSection2:Cell("B1_XUBICA1"):SetSize(08)
		oSection2:Cell("B1_XUBICA2"):SetSize(08)
	...
	BeginSql Alias cAliasTop
		...
		WHERE
			...
			AND SC2.C2_NUM || SC2.C2_ITEM || SC2.C2_SEQUEN >= %Exp:cWhere01%
			AND SC2.C2_NUM || SC2.C2_ITEM || SC2.C2_SEQUEN <= %Exp:cWhere02%
			AND SC2.C2_DATPRF BETWEEN %Exp:mv_par03% AND %Exp:mv_par04%
			AND SC2.%NotDel% %Exp:cWhere03%
		ORDER BY
			%Exp:cOrderby%
	EndSql
	oSection1:EndQuery()
	...
	While !oReport:Cancel() .And. !(cAliasTop)->(Eof())
		...
		oReport:SetTitle('Orden de Producción Nro '+(cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)) //"        O R D E M   D E   P R O D U C A O       NRO :"

		If mv_par06 == 1
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Verifica se o relatório é local ou servidor.³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			If oReport:nEnvironment == 1 .And. oReport:nDevice <> 6
				nLinBar := 1.0
			Else
				nLinBar := 2.5
			EndIf
			...
			While SG2->(!Eof()) .And. Eval(&cSeekWhile)
				...
				If lSH8
					While SH8->(!Eof()) .And. SH8->(H8_FILIAL+H8_OP+H8_OPER) == xFilial("SH8")+(cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD)+SG2->G2_OPERAC
						...
						IF nCont >= 1
							oSection3:PrintHeader()//Imprime o cabeçalho da seção
						ENDIF
						oSection3:PrintLine()
						oSection4:PrintLine()
						oReport:ThinLine()//Desenha uma linha simples iniciando na linha atual de impressão com a largura da página
						oReport:SkipLine()//Salta uma linha baseado na altura da linha informada pelo usuário
						nCont ++
						SH8->(dbSkip())
					EndDo
				Else
...
Static Function R820Imp(lEnd,wnRel,titulo,tamanho)
	...
	If	Upper(TcGetDb()) $ 'ORACLE,DB2,POSTGRES,INFORMIX'
		cQuery += "SC2.C2_NUM || SC2.C2_ITEM || SC2.C2_SEQUEN >= '" + AllTrim(mv_par01) + "' AND "
		cQuery += "SC2.C2_NUM || SC2.C2_ITEM || SC2.C2_SEQUEN <= '" + AllTrim(mv_par02) + "' AND "
	Endif
	...
	While !Eof()
		...
		For I := 2 TO Len(aArray)
			...
			If mv_par12 == 2
				...
				@li ,  65 PSay Substr(aArray[I][8],1,10)            // ENDEREÇO
				@li ,  75 PSay " |"+aArray[I][9]                  	// SEQUENCIA
			Else
				@Li ,  59 PSay "| "+aArray[I][4]+" |"			  	// UNIDADE DE MEDIDA
				@li ,  65 PSay aArray[I][6]+"  |"                  	// ALMOXARIFADO
				@li ,  70 PSay Substr(aArray[I][8],1,10)            // ENDEREÇO
				@li ,  83 PSay " | "+aArray[I][9]                  	// SEQUENCIA
				@li ,  89 PSay " | "+aArray[I][11]                  // LOTE
				@li , 102 PSay " | "+aArray[I][12]                  // SUB-LOTE
			EndIf
...
Static Function Verilim()
	...
	IF Li > 59						// Li > 55
		Li := 0
		cRotOper()					// Imprime cabecalho roteiro de operacoes
	Endif
Return Li
...
Static Function _xDesg1(cXProdu,cXComp)
	Local _xCprodu := cXProdu
	Local _cAQuery := ""
	Local cObs := ""
	Local cRetorno :=  ""
...
Static Function _xRecu1(rXProdu,rXComp,rXFilial,rXreteiro,xNum,xSeq)
	...
	Local cRecur := ""
	Local cRetorno :=  ""
	Local _rXComp := rXComp
	Local _rXreteiro := rXreteiro

	_cAQuery :="SELECT SG2.G2_RECURSO FROM "+RetSQLName('SG2')+" SG2 "
	_cAQuery +="INNER JOIN "+RetSQLName('SC2')+" SC2 "
	_cAQuery +="ON SG2.G2_CODIGO=SC2.C2_ROTEIRO "
	_cAQuery +="AND SG2.G2_PRODUTO = SC2.C2_PRODUTO "
	_cAQuery +="INNER JOIN "+RetSQLName('SGF')+" SGF "
	_cAQuery +="ON SGF.GF_PRODUTO=SG2.G2_PRODUTO "
	_cAQuery +="AND SGF.GF_OPERAC=SG2.G2_OPERAC "
	_cAQuery +="AND SGF.GF_ROTEIRO=SC2.C2_ROTEIRO "
	_cAQuery +="WHERE  "
	_cAQuery +="G2_PRODUTO  = '"+ALLTRIM(_rXCprodu)+"' "
	_cAQuery +="AND GF_COMP ='"+ALLTRIM(_rXComp)+"' "
	_cAQuery +="AND SGF.GF_FILIAL='"+ALLTRIM(_rXFilial)+"' "
	_cAQuery +="AND SGF.GF_ROTEIRO='"+ALLTRIM(_rXreteiro)+"' "
	_cAQuery +="AND SGF.D_E_L_E_T_ <> '*'  "
	_cAQuery +="AND SG2.D_E_L_E_T_ <> '*'  "
	_cAQuery +="AND SC2.D_E_L_E_T_ <> '*'  "

	TcQuery _cAQuery New Alias "_aQRY"
	dbSelectArea("_aQRY")
	While !_aQRY->(EOF())
		cRecur := _aQRY->G2_RECURSO
		_aQRY->(dbSkip())
	EndDo
	_aQRY->(dbCloseArea())
