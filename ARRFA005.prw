...
If FWCodEmp() = '01'
	...
	SELECT  %Exp:cEmpAnt% AS EMPRESA, D2_FILIAL AS FILIAL, D2_EMISSAO AS FECHA, D2_CLIENTE AS CLIENTE, A1_NOME AS RAZON_SOCIAL, A3_NOME AS VENDEDOR, A3_COD AS CODVENDEDOR,
			...
			D2_PEDIDO AS PEDIDO , D2_LOJA AS LOJA  , D2_VALIMP9 AS AUTO_RETENCION_ESPECIAL ,D2_VALIMP6 AS AUTORETENCION_VENTA, 
			D2_VALIMP5 AS AUTORETENCION_HONORARIOS , D2_VALIMPA AS AUTORETENCION_ICA , F2_DTLANC AS DTLANC 
		FROM %table:SD2% SD2
   ...
   SELECT %Exp:cEmpAnt% AS EMPRESA,D1_FILIAL AS FILIAL,D1_EMISSAO AS FECHA,D1_FORNECE AS CLIENTE,A1_NOME AS RAZON_SOCIAL, 
		  ...
		  D1_PEDIDO AS PEDIDO , D1_LOJA AS LOJA , D1_VALIMP9*(-1) AS AUTO_RETENCION_ESPECIAL , D1_VALIMP6*(-1) AS AUTORETENCION_VENTA, 
		  D1_VALIMP5*(-1) AS AUTORETENCION_HONORARIOS , D1_VALIMPA*(-1) AS AUTORETENCION_ICA , F1_DTLANC AS DTLANC 
	 FROM %table:SD1% SD1
