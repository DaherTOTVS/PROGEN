...
Function M486XMLHKA(cPrejixo, aFact, nPos, aError, lMailAut, lNoEnv, cFilSA, cFilAI0,cEmailEmp)
	...
	Local lCpoMues   := SD2->(ColumnPos("D2_NICO")) > 0
	...
	If oXML <> Nil	
		...
		IF lCpoMues .And. lCpoAsum .And. Alltrim(AI0->AI0_TS) == "N"
			M486DESGRA(xFilial("SF2"), aFact[nPos,1],aFact[nPos,2],aFact[nPos,3],aFact[nPos,4],@aDesc)
...
Function M486DETDOC(oWS, aOpcDoc, cObjXML,aFact,nPosDoc,cTpOper,lCpoMues)
	...
	If AllTrim(cEspecie) $ "NF" .And. lCpoMues
		cProd := PadR(&(cObjXML + ":" + aOpcDoc[4] + ":_CAC_SELLERSITEMIDENTIFICATION:_CBC_ID:TEXT"),FWSX3Util():GetFieldStruct( "D2_COD" )[3])
		cItem := IIf( Len(&(cObjXML + ":_CBC_ID:TEXT")) == 1 ,"0","")+&(cObjXML + ":_CBC_ID:TEXT")
		SD2->(dbSetOrder(3)) //D2_FILIAL + D2_DOC + D2_SERIE + D2_CLIENTE + D2_LOJA + D2_COD + D2_ITEM
		If SD2->(Msseek(xFilial("SD2") + aFact[nPosDoc,2] + aFact[nPosDoc,1]+ aFact[nPosDoc,3] + aFact[nPosDoc,4]+cProd+cItem))
			If Alltrim(SD2->D2_NICO) == "S"
				lMuestra:=.T.
			EndIf
		EndIF
	EndIf
...
Function M486GETTAX(cDoc, cSerie,cCliente, cLoja,cEsp)
	...
	ElseIf cEsp $ "NCC|NDP"
		BeginSql alias cTmp
			SELECT DISTINCT FB_CODIGO,FB_IMPSAT, FB_CLASSE,FB_CPOLVRO
			FROM %table:SFB% SFB INNER JOIN %table:SFC% SFC ON FB_CODIGO=FC_IMPOSTO
			WHERE FB_FILIAL = %exp:xfilial("SFB")%
			AND FC_FILIAL = %exp:xfilial("SFC")%
			AND SFC.%notDel%
	...
	While (!(cTmp)->(EOF()))
		...
		6. Valor Impuesto.
		*/
		AADD( aImp , { (cTmp)->FB_IMPSAT,(cTmp)->FB_CODIGO,(cTmp)->FB_CLASSE,FB_CPOLVRO,0.00,0.00,IIf(cEsp $ "NF",(cTmp)->FC_TES,"")})
		(cTmp)->(dbskip())
	EndDo
...
/*/{Protheus.doc} M486DESGRA
	...
	/*/
Function M486DESGRA(cFilDoc, cSerie, cDoc, cCliente, cLoja,aDesc)
	Local aArea		:= GetArea()
	LOCAL nTotal 	:= 0
	LOCAL aImp 		:= {}
	LOCAL cChave	:= ""
	...
	SD2->(dbSetOrder(3)) //D2_FILIAL + D2_DOC + D2_SERIE + D2_CLIENTE + D2_LOJA + D2_COD + D2_ITEM
		IF (SD2->(Msseek( cChave )) )
			...
			While SD2->(!Eof()) .And. cChave == cFilDoc + SD2->D2_DOC + SD2->D2_SERIE + SD2->D2_CLIENTE + SD2->D2_LOJA
				For nX := 1 To Len(aImp)
					If Alltrim( aImp[nX,1] ) == "01" .And. ((cAlias)->&("D2_TES") == aImp[nX,7])
						If AllTrim(SD2->D2_NICO) == "S"
							nVImpMues +=  (cAlias)->&("D2_VALIMP" + aImp[nX,4])
							lRet := .T.
						Else
							nBaseImp := (cAlias)->&("D2_BASIMP" + aImp[nX,4])
							nValImp  :=  (cAlias)->&("D2_VALIMP" + aImp[nX,4])
